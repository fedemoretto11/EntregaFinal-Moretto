  ...
  return (
    <article key={producto.id} className='card h-[20rem] shadow-md p-4 duration-300 transition flex flex-col bg-white hover:bg-slate-200'>
      <Link to={`/product/${producto.id}`} className='h-full'>
        ...
        <h3 className='card__price h-1/6 text-start pt-2 text-xl'>{`${producto.price.toLocaleString("es-AR",{style:"currency", currency:"ARS"})}`}</h3>
        <p className='card__description h-1/6 text-start text-ellipsis overflow-hidden truncate'>{producto.title}</p>
      </Link>
    </article>
  )
}
